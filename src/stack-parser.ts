...
export function parseStack(stack: string): ParsedStack {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    let matchedType: 'chrome' | 'chrome_short' | 'firefox' | 'safari' | 'safari_short' | null = null
    
    match = lineText.match(CHROME_REGEX)
    if (match) matchedType = 'chrome'
    
    if (!match) {
      match = lineText.match(CHROME_SHORT_REGEX)
      if (match) matchedType = 'chrome_short'
    }
    
    if (!match) {
      match = lineText.match(FIREFOX_REGEX)
      if (match) matchedType = 'firefox'
    }
    
    if (!match) {
      match = lineText.match(SAFARI_REGEX)
      if (match) matchedType = 'safari'
    }
    
    if (!match) {
      match = lineText.match(SAFARI_SHORT_REGEX)
      if (match) matchedType = 'safari_short'
    }
